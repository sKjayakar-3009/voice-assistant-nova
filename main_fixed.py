...
def get_path(filename):
    return os.path.join(BASE_DIR, "photos", filename)
...
def start_captioning(root):
    ...
    recognizer = sr.Recognizer()
    def update_caption():
        if not caption_window.winfo_exists():
            return

        with sr.Microphone() as source:
            caption_label.config(text="Listening...")
            try:
                audio = recognizer.listen(source)
                text = recognizer.recognize_google(audio)
        ...
        if caption_window.winfo_exists():
            caption_window.after(2000, update_caption)
...
def handle_task(r, update_ui_callback, stop_main_mic, input_text=None, user_input=None):
    while True:
        ...
        def handle_next_task(ready_button):
            ...
            start_listening(update_ui_callback)
...
def open_translation_window(update_ui_callback):
    ...
    mic_image = Image.open(get_path("mic.png"))
    ...
    lang_var = StringVar()
    lang_var.set("தமிழ்")
    lang_options = ["Spanish", "French","தமிழ்","Chinese", "Japanese"]
...
def open_reminder_window(update_ui_callback):
    ...
    reminder_window.configure(bg="#ffebcd")
...
class VoiceAssistantApp:
    def __init__(self, root):
        ...
        self.video_path = get_path("back.mp4")
    ...
    def create_profile_card(self):
        ...
        try:
            img = Image.open(get_path("user.png"))
    ...
    def setup_gui(self):
        self.ellie_image_path = get_path("nova.jfif")
        self.user_image_path = get_path("user.png")
        ...
        ready_button = tk.Button(
            self.root, text="Ready", command=self.start_voice_input,
